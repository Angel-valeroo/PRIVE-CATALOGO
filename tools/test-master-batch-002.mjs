...
const expectedAll = legacy.slice(0, 150);
const errors = [];

if (manifest.batchSize !== 50) errors.push("batchSize general debe conservarse en 50");
if (manifest.activeBatchSize !== 100) errors.push("activeBatchSize debe ser 100 para el lote 002 autorizado");
if (manifest.activeBatch !== 2) errors.push("activeBatch debe ser 2");
if (!Array.isArray(manifest.perfumes) || manifest.perfumes.length !== 150) errors.push("El manifiesto debe activar 150 fichas en total");
if (manifest.batches?.length !== 2) errors.push("El manifiesto debe contener exactamente dos lotes");
if (manifest.batches?.[0]?.count !== 50) errors.push("batch-001 debe conservar 50 fichas");
if (manifest.batches?.[1]?.count !== 100) errors.push("batch-002 debe declarar 100 fichas");
...
if (JSON.stringify(manifest.batches?.[1]?.codes || []) !== JSON.stringify(expectedSecond.map(x => x.code))) errors.push("Los códigos del lote 002 no coinciden con las posiciones 51–150");
...
for (const filename of manifest.perfumes || []) {
  ...
  if (expectedSecond.some(item => item.code === code)) {
    if (perfume.status !== "review") errors.push(`${code}: el lote 002 debe estar en review`);
    const titles = (perfume.provenance?.sources || []).map(source => source.title || "");
...
if (JSON.stringify(loadedCodes) !== JSON.stringify(expectedAll.map(x => x.code))) errors.push("El orden activo no coincide con los primeros 150 perfumes");

const report = "data/core/review-batch-002.csv";
if (!fs.existsSync(report)) errors.push("Falta review-batch-002.csv");
else {
...
console.log("✅ Lote maestro 002 validado: 100 fichas nuevas, 150 activas, orden correcto y fuentes duales registradas");
