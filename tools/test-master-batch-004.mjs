...
const expectedAll = legacy.slice(0, 350);
const errors = [];
...
if (manifest.activeBatch !== 4) errors.push("activeBatch debe ser 4");
if (!Array.isArray(manifest.perfumes) || manifest.perfumes.length !== 350) errors.push("El manifiesto debe activar 350 fichas en total");
if (manifest.batches?.length !== 4) errors.push("El manifiesto debe contener exactamente cuatro lotes");
...
for (const filename of manifest.perfumes || []) {
  ...
  if (expectedFourth.some(item => item.code === code)) {
    if (perfume.status !== "review") errors.push(`${code}: el lote 004 debe estar en review`);
...
const report = "data/core/review-batch-004.csv";
if (!fs.existsSync(report)) errors.push("Falta review-batch-004.csv");
else {
...
console.log("✅ Lote maestro 004 validado: 100 fichas nuevas, 350 activas, orden correcto y fuentes duales registradas");
