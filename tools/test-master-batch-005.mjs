...
const expectedAll = legacy.slice(0, 450);
const errors = [];

if (manifest.batchSize !== 50) errors.push("batchSize general debe conservarse en 50");
if (manifest.activeBatchSize !== 100) errors.push("activeBatchSize debe ser 100 para el lote 004 autorizado");
if (manifest.activeBatch !== 5) errors.push("activeBatch debe ser 5");
if (!Array.isArray(manifest.perfumes) || manifest.perfumes.length !== 450) errors.push("El manifiesto debe activar 450 fichas en total");
if (manifest.batches?.length !== 5) errors.push("El manifiesto debe contener exactamente cinco lotes");
...
if (JSON.stringify(manifest.batches?.[4]?.codes || []) !== JSON.stringify(expectedFifth.map(x => x.code))) errors.push("Los códigos del lote 005 no coinciden con las posiciones 351–450");
...
for (const filename of manifest.perfumes || []) {
  ...
  if (expectedFifth.some(item => item.code === code)) {
    if (perfume.status !== "review") errors.push(`${code}: el lote 005 debe estar en review`);
...
const report = "data/core/review-batch-005.csv";
if (!fs.existsSync(report)) errors.push("Falta review-batch-005.csv");
else {
...
console.log("✅ Lote maestro 005 validado: 100 fichas nuevas, 450 activas, orden correcto y fuentes duales registradas");
